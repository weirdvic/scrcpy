...
[Читать на других языках](#translations)

Это приложение предоставляет возможность отображения экрана и управления
Android устройствами, подключенными через USB (или [по TCP/IP][article-tcpip]).
Для работы не требуется _root_ доступ.
Приложение работает на _GNU/Linux_, _Windows_ и _macOS_.

![screenshot](assets/screenshot-debian-600.jpg)

Основные приоритеты:

 - **легковесность** (отображается только экран устройства)
 - **производительность** (30~60fps)
 - **качество** (разрешение 1920×1080 или выше)
 - **низкие задержки** ([35~70ms][lowlatency])
 - **быстрый запуск** (~1 секунда до появления изображения с устройства)
 - **не оставляет следов** (на управляемое устройство ничего не устанавливается)
...
Устройство Android с поддержкой API 21 (Android 5.0).

Убедитесь что на устройстве [включена отладка по adb][enable-adb].

[enable-adb]: https://developer.android.com/studio/command-line/adb.html#Enabling

На некоторых устройствах дополнительно нужно [разрешить доступ][control] для
управления устройством клавиатурой и мышью.
...
Для Debian (_testing_ и _sid_) и Ubuntu (20.04):

```
apt install scrcpy
```

Доступен [Snap] пакет: [`scrcpy`][snap-link].

[snap-link]: https://snapstats.org/snaps/scrcpy

[snap]: https://en.wikipedia.org/wiki/Snappy_(package_manager)

Для Fedora доступен [COPR] пакет: [`scrcpy`][copr-link].

[COPR]: https://fedoraproject.org/wiki/Category:Copr
[copr-link]: https://copr.fedorainfracloud.org/coprs/zeno/scrcpy/

Для Arch Linux доступен [AUR] пакет: [`scrcpy`][aur-link].

[AUR]: https://wiki.archlinux.org/index.php/Arch_User_Repository
[aur-link]: https://aur.archlinux.org/packages/scrcpy/

Для Gentoo доступен [Ebuild]: [`scrcpy/`][ebuild-link].

[Ebuild]: https://wiki.gentoo.org/wiki/Ebuild
[ebuild-link]: https://github.com/maggu2810/maggu2810-overlay/tree/master/app-mobilephone/scrcpy

Также возможно [собрать приложение самостоятельно][BUILD] (не волнуйтесь, это не настолько сложно).



### Windows

Ради простоты использования, для Windows доступен архив с уже собранной версией и
всеми зависимостями, включая `adb`:
...
Также приложение доступно в [Chocolatey]:

[Chocolatey]: https://chocolatey.org/

```bash
choco install scrcpy
choco install adb    # если не установили ранее
```
...
scoop install adb    # если не установили ранее
```

[Scoop]: https://scoop.sh

Наконец, возможно [собрать приложение самостоятельно][BUILD].


### macOS

Приложение доступно в [Homebrew]. Просто установить:
...
Команда `adb` должна быть доступна в `PATH`. Если не делали этого раньше, то:
...
Возможно [собрать приложение самостоятельно][BUILD].


## Запуск

Подключите Android и выполните на компьютере:

```bash
scrcpy
```

Чтобы посмотреть доступные аргументы командной строки:
